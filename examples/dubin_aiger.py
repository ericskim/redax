...
for numapplied in range(10):
    ...
    dubins_x = dubins_x.io_refined({k: v for k, v in iobox.items() if k in dubins_x.vars}, nbits=precision)
    dubins_y = dubins_y.io_refined({k: v for k, v in iobox.items() if k in dubins_y.vars}, nbits=precision)
    dubins_theta = dubins_theta.io_refined({k: v for k, v in iobox.items() if k in dubins_theta.vars}, nbits=precision)


    # dubins_x.pred.aag = BoolExpr(aig=simplify(dubins_x.pred.aag.aig))
    # dubins_y.pred.aag = BoolExpr(aig=simplify(dubins_y.pred.aag.aig))
    # dubins_theta.pred.aag = BoolExpr(aig=simplify(dubins_theta.pred.aag.aig))
